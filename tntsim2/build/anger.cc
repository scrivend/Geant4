...
private:
	AngerCamera(const AngerCamera&) :
		...
		kPrcntAnthracene(1), kAttenLength(160), kTimeRes(0),
		//by Shuya 160427
		//kX(200), kY(200), kZ(30)
...
public:
    ...
    //Edit by Dustin to pTer light output 180525
	static const Int_t kNumAnthracene = (int)(27000.*0.2); //!
...
class AngerCameraAnalyzer :
	//by Shuya 180516. For some reason, the code didn't work with "private" on gca server. (worked on guardia.tamu.edu).
	//private AngerCamera {
	public AngerCamera {
public:
	AngerCameraAnalyzer():
...
void anger(int nthreads = 1)
{
	...
	//Int_t bins[] = { 8 }; // 5, 8, 10, 15, 30 };
	Int_t bins[] = { 4 }; // 5, 8, 10, 15, 30 };
	int meth [] = {4} ; //, 5, 0};

	TFile* fileSim = TFile::Open("/home/scrivend/Geant4/tntsim2/build/LXeDataTree.root");
