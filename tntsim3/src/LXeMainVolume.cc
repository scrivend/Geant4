...
LXeMainVolume::LXeMainVolume(G4RotationMatrix *pRot,
  ...
  :G4PVPlacement(pRot,tlate,
                 ...
                 //Comment by Shuya 160609. System units of lengh in Geant4 is mm, so this G4Box is really tiny relative to the housing (in order of m), so unecessary to remove this temp box. (or probably ok to make it much smaller) 
...
//by Shuya 160608. I moved this here from below.
  //****************** Build PMTs
...
//by Shuya 160608. I added below to put light guide in front of PMTs.
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  fPmt_hollow = new G4Box("pmthollow_box",fPmt_x/2.,fPmt_y/2.,height_pmt);
  //G4ThreeVector	translation(-37.5*cm, 37.5*cm, housing_z/2.-height_pmt/2.);
  hollowHousing_box = new G4Box("HollowBox_S",housing_x/2., housing_y/2., housing_z/2.);

//by Shuya 160608. I moved these to here from below.  
  ...
  PlacePMTs2(fPmt_hollow,0,x,y,dx,dy,xmin,ymin,fNx,fNy,x,y,z,k);

  G4RotationMatrix* rm_z = new G4RotationMatrix();
  rm_z->rotateY(180*deg);
  z = fScint_z/2. + height_pmt;       //back
  PlacePMTs2(fPmt_hollow,rm_z,x,y,dx,dy,xmin,ymin,fNx,fNy,x,y,z,k);
 
//Put light guides only front and back side of the scintillator because on other sides, the surface size doesn't necessarily match with scintillator surface.
/*
  G4RotationMatrix* rm_y1 = new G4RotationMatrix();
  rm_y1->rotateY(-90*deg);
  x = -fScint_x/2. - height_pmt;      //left
  PlacePMTs2(fPmt_hollow,rm_y1,y,z,dy,dz,ymin,zmin,fNy,fNz,x,y,z,k);

  G4RotationMatrix* rm_y2 = new G4RotationMatrix();
  rm_y2->rotateY(90*deg);
  x = fScint_x/2. + height_pmt;      //right
  PlacePMTs2(fPmt_hollow,rm_y2,y,z,dy,dz,ymin,zmin,fNy,fNz,x,y,z,k);
 
  G4RotationMatrix* rm_x1 = new G4RotationMatrix();
  rm_x1->rotateX(90*deg);
  y = -fScint_y/2. - height_pmt;     //bottom
  PlacePMTs2(fPmt_hollow,rm_x1,x,z,dx,dz,xmin,zmin,fNx,fNz,x,y,z,k);

  G4RotationMatrix* rm_x2 = new G4RotationMatrix();
  rm_x2->rotateX(-90*deg);
  y = fScint_y/2. + height_pmt;      //top
  PlacePMTs2(fPmt_hollow,rm_x2,x,z,dx,dz,xmin,zmin,fNx,fNz,x,y,z,k);
*/
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  //fHousing_log = new G4LogicalVolume(fHousing_box,
  				//by Shuya 160613.
  fHousing_log = new G4LogicalVolume(hollowHousing_box,
...
*/
  ...
  xmin = -fScint_x/2. - dx/2.;
  ymin = -fScint_y/2. - dy/2.;
  zmin = -fScint_z/2. - dz/2.;
  k=0;
 
  z = -fScint_z/2. - height_pmt;      //front
  //PlacePMTs(fPmt_log,0,x,y,dx,dy,xmin,ymin,fNx,fNy,x,y,z,k);
//by Shuya 160609
  //PlacePMTs(fPmt_log,0,x,y,dx,dy,xmin,ymin,fNx,fNy,x,y,z,k,fHousing_log);
  //by Shuya 160613. I confirmed the above function worked so made it to the same condition as backside (using Boolean Subtraction Geometry)
  PlacePMTs(fPmt_log,0,x,y,dx,dy,xmin,ymin,fNx,fNy,x,y,z,k,pMotherLogical);
...
//by Shuya 160609
  PlacePMTs(fPmt_log,rm_z,x,y,dx,dy,xmin,ymin,fNx,fNy,x,y,z,k,pMotherLogical);
//by Shuya 160428. You can check PMT position by this.
  //for(int i=100;i<200;i++)	G4cout << fPmtPositions[i] << G4endl;
 
/*by Shuya 160609. Decided not to put PMTs except front and back sides because I didn't use them for counting photons so far. 

//by Shuya 160608.
  //G4RotationMatrix* rm_y1 = new G4RotationMatrix();
  rm_y1 = new G4RotationMatrix();
  rm_y1->rotateY(-90*deg);
  x = -fScint_x/2. - height_pmt;      //left
  ...
  for(G4int j=1;j<=na;j++){
    ...
    for(G4int i=1;i<=nb;i++){
      ...
      new G4PVPlacement(rot,G4ThreeVector(x,y,z),pmt_log,"pmt",
                        ...
                        //by Shuya 160609. PMT could prorude from Housing box, so change the mother volume to Experimental hall.
                        pMotherLogical,false,k);
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
//By Shuya 160608. To make spaces for PMTSs inside the (scintillation) Housing box.
void LXeMainVolume::PlacePMTs2(G4VSolid* pmt_hollow,
                              G4RotationMatrix *rot,
                              G4double &a, G4double &b, G4double da,
                              G4double db, G4double amin,
                              G4double bmin, G4int na, G4int nb,
...
*/
  ...
  for(G4int j=1;j<=na;j++){
    ...
    for(G4int i=1;i<=nb;i++){
      ...
      //new G4PVPlacement(rot,G4ThreeVector(x,y,z),pmt_log,"pmt",
      //                  fHousing_log,false,k);
      sprintf(brN, "HollowBox_%d", i);
      //hollowHousing_box = new G4SubtractionSolid(brN,hollowHousing_box, pmt_hollow, 0, G4ThreeVector(x,y,z));
      hollowHousing_box = (G4VSolid*)new G4SubtractionSolid(brN,hollowHousing_box, pmt_hollow, 0, G4ThreeVector(x,y,z));
      //fPmtPositions.push_back(G4ThreeVector(x,y,z));
      //k++;
    }
  }
